...
using ComicBooks.Services;
using ComicBooks.ViewModels.Account;
...
namespace ComicBooks.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost("theme")]
        [Authorize]
        public async Task<IActionResult> ChooseTheme([FromBody]Theme theme)
        {
            var found = false;
            ...
            foreach (Claim claim in currentClaims)
            {
                if (claim.Type == "theme")
                {
                    await _userManager.ReplaceClaimAsync(user, claim, new Claim("theme", theme.ThemeFile));
                    found = true;
                }
            }
            if (!found)
            {
                await _userManager.AddClaimAsync(user, new Claim("theme", theme.ThemeFile));
            }
            return Ok();
        }
        ...
        public async Task<IActionResult> MakeAdmin([FromBody]UserViewModel user)
        {
            var fullUser = await _userManager.FindByNameAsync(user.UserName);
            await _userManager.AddClaimAsync(fullUser, new Claim("IsAdmin", "true"));
            return Ok();
        }
